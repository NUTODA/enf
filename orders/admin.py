...
class OrderItemInline(admin.TabularInline):
    ...
    def image_preview(self, obj):
        if obj.product.main_image:
            return mark_safe(
                f'<img src="{obj.product.main_image.url}" style="max_height: 100; "max_width: 100px; object-fit: cover;"/>'
            )
        return mark_safe(f'<span style="color gray;">No Image</span>')
    ...
    fieldsets = (
        ("Order Information", {
            "fields": (
                ...
                "email", "company", "address1", "address2",
        ...
        ("Payment and Status", {
            "fields": (
                "status", "payment_provider", "stripe_payment_intent_id"
            )
        }),
        ("Timestamps", {
            "fields": (
