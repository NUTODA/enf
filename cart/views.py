...
class CartModalView(CartMixin, View):
    def get(self, request):
        ...
        context = {
            "cart": cart,
            "cart_items": cart.items.select_related("product", "product_size").order_by(
                "-added_at"
            ),
        }
...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request, slug):
        ...
        if size_id:
            ...
            if not product_size:
                return JsonResponse({"error": "No sizes available"}, ststus=400)

        quantity = form.cleaned_data["quantity"]
        if product_size.stock < quantity:
            return JsonResponse(
                {"error": f"Only {product_size.stock} items available"}, ststus=400
            )
            ...
            return JsonResponse(
                {
                    ...
                    "message": f"{product.name} added in cart",
